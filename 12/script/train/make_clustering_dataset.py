...
def save_candlestick_img_only_ma(df, save_filepath):
    ...
    ax.plot(x_list, open_value, markersize=2, color='y')
    ax.plot(x_list, close_value, markersize=2, color='black')
    ax.plot(x_list, high_value, markersize=2, color='r')
    ax.plot(x_list, low_value, markersize=2, color='b')
    ax.grid(False)
    ax.set_xticklabels([])
    ax.set_yticklabels([])
